...
class myGATConv(nn.Module):
    ...
    def forward(self, graph, feat, e_feat, res_attn=None):
        with graph.local_scope():
            ...
            # self.edge_emb.weight shape: [num_etypes, edge_feats]
            # w_trans shape: [num_etypes, num_heads, edge_feats]
            w_trans = self.fc_e(self.edge_emb.weight).view(-1, self._num_heads, self._edge_feats)

            # self.attn_e shape: [1, num_heads, edge_feats]
            # ee_per_type shape: [num_etypes, num_heads]
            ee_per_type = (w_trans * self.attn_e).sum(dim=-1)

            # e_feat shape: [num_edges]
            # ee shape: [num_edges, num_heads]
            ee = F.embedding(e_feat, ee_per_type)

            # shape: [num_edges, num_heads, 1]
            ee = ee.unsqueeze(-1)
