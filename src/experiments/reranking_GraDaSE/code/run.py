...
def run_model_FAERY(args):
    ...
    cache_path = get_cache_filename(args)
    
    if os.path.exists(cache_path) and args.checkpoint:
        print(f"Loading cached data from {cache_path} ...")
        try:
            cache = torch.load(cache_path)
            
            features_list = [f.to(device) for f in cache['features_list']]
            e_feat = cache['e_feat'].to(device)
            type_emb = cache['type_emb'].to(device)
            node_type = cache['node_type'].to(device)
            test_batches = cache['test_batches']
            in_dims = cache['in_dims']
            node_cnt = cache['node_cnt']
            edge_links_count = cache.get('edge_links_count', len(dl.links['count'])) 
            
            print("Data loaded successfully! Skipping preprocessing.")
            ...
            ranker = GraDaSE(g=g, 
                            edge_dim=64,
                            num_etypes=edge_links_count * 2 + 1,
                            num_class=2, 
                            input_dimensions=in_dims, 
                            embeddings_dimension=args.hidden_dim,
            ...
            ranker.load_state_dict(torch.load(args.checkpoint))
            
            inference(ranker, test_batches, features_list, e_feat, type_emb, node_type, device, args, dl)
            return
            
        except Exception as e:
            print(f"Failed to load cache: {e}. Fallback to normal processing.")
    ...
    if not os.path.exists(cache_path):
        ...
        torch.save({
            ...
            'in_dims': in_dims,
            'node_cnt': node_cnt,
            'edge_links_count': len(dl.links['count'])
        }, cache_path)
        print("Data saved!")
    ...
    for i in range(args.repeat):
        exp_id = get_experiment_id(args, i)
        ...
        early_stopping = EarlyStopping(
            ...
            save_path=f'checkpoint/GraDaSE_{exp_id}.pt')
        early_stop = False
        steps = 0
        ...
        ranker.load_state_dict(torch.load(f'checkpoint/GraDaSE_{exp_id}.pt'))
        ranker.eval()
        with torch.no_grad():
            ...
            print(f"Repeat: {i} ({exp_id})")
            for metric in metrics:
                test_results[metric][i] = result[metric]
                print(f'{metric}: {result[metric]:.4f}', end='\t')
            print('\n')

            with open(f'result/result_{exp_id}.json', 'w') as f:
                json.dump(result, f)
            with open(f'result/run_{exp_id}.json', 'w') as f:
                json.dump(run, f)
    for metric in metrics:
        print(f"{metric}: {test_results[metric].mean().item():.4f}, std: {test_results[metric].std().item():.4f}", )


def get_experiment_id(args, run_idx):
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    
    # dataset, num_layers, num_gnns, len_seq, lr, batch_size, num_seqs, top_k, i
    param_str = (
        f"{args.dataset}_{args.num_layers}_{args.num_gnns}_{args.len_seq}_"
        f"{args.lr}_{args.batch_size}_{args.num_seqs}_{args.top_k}_{args.dropout}_{args.weight_decay}_{run_idx}"
    )
...
def get_cache_filename(args):
    """
    processed_{dataset}_ft{feats_type}_rn{retrieve_num}_k{top_k}_ls{len_seq}_ns{num_seqs}_bs{batch_size}.pt
    """
    cache_name = (
        f"processed_{args.dataset}"
        f"_ft{args.feats_type}"
        f"_rn{args.retrieve_num}"
        f"_k{args.top_k}"
        f"_ls{args.len_seq}"
        f"_ns{args.num_seqs}"
    ...
    return os.path.join('../data', args.dataset, cache_name) 

def inference(ranker, test_batches, features_list, e_feat, type_emb, node_type, device, args, dl):
    ...
    test_json_path = f'../data/{args.dataset}/test.json'
    if os.path.exists(test_json_path):
        with open(test_json_path, 'r') as f:
            qrels = json.load(f)
    else:
        print(f"Warning: {test_json_path} not found. Evaluation metrics might be skipped.")

    with torch.no_grad():
        for batch in tqdm(test_batches, desc="Ranking"):
            scores = ranker(features_list,
                            e_feat,
                            batch['qt_seq'], type_emb, node_type,
                            [batch['ct_seqs'], batch['mask_pos'], batch['dataset_input_ids'].to(device)],
            ...
            for id_ in range(len(batch['qt_seq'])):
                pair_id = batch['pair_id'][id_]
                dataset_id = batch['dataset'][id_]
                score = scores[id_]
                
                if pair_id not in run:
                    run[pair_id] = {}
                run[pair_id][str(dataset_id)] = score
    ...
    match = re.search(r'GraDaSE_(.+)\.pt$', args.checkpoint)
    if match:
        name = match.group(1)
    else:
        name = os.path.splitext(os.path.basename(args.checkpoint))[0]
    result_file = f'result/result_{name}.json'
    run_file = f'result/run_{name}.json'
...
if __name__ == '__main__':
    ...
    ap.add_argument('--checkpoint', type=str, default=None, help='Path to the checkpoint file')
